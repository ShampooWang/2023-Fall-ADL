...
import json
from utils import get_prompt, get_bnb_config, create_fewshot_instruction, get_prompt_with_fewshots
import argparse


def perplexity(
    model, tokenizer, data_path="/work/jgtf0322/Homework/2023-Fall-ADL/HW3/dataset/public_test.json", max_length=2048, fewshots=None
):
    ...
    data_size = len(data)
    instructions = [get_prompt_with_fewshots(x["instruction"], fewshots=fewshots) for x in data]
...
if __name__ == "__main__":
    ...
    model.eval()
    fewshots = create_fewshot_instruction(args.shot_num, "/work/jgtf0322/Homework/2023-Fall-ADL/HW3/dataset/train.json")
    ppl = perplexity(model, tokenizer, args.test_data_path, fewshots=fewshots)
