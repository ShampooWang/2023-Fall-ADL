...
def main():
    ...
    def postprocess_text(preds, labels):
        ...
        preds = ["\n".join(nltk.sent_tokenize(pred)) for pred in pred_list ]
        labels = ["\n".join(nltk.sent_tokenize(label)) for label in label_list ]
    ...
    # Configuration for generation, default is greedy. Details: https://huggingface.co/docs/transformers/v4.35.0/en/main_classes/text_generation#transformers.GenerationConfig
    generation_config = {
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                output_prediction_file = os.path.join(training_args.output_dir, "generated_predictions.txt") if model_args.output_file is None else model_args.output_file 
                with jsonlines.open(output_prediction_file, "w") as writer:
                    for pred, p_id in zip(predictions, raw_datasets["test"]["id"]):
                        writer.write({"title": pred, "id": p_id})
